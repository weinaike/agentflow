...
from ..prompt_template import TASK_TEMPLATE,CHECK_SYSTEM_PROMPT,CHECK_TEMPLATE,REVISE_TEMPLATE

class SelectorGroupChatNode(AgentNode):
    def __init__(self, config: Union[Dict, AgentNodeParam]):
        super().__init__(config)
        self.questions = self._node_param.manager.questions
        if len(self.questions) == 0:
            raise ValueError("SelectorGroupChatNode must have at least one question")
        ...
        self.team = SelectorGroupChat(participants = self.agents, 
                                      model_client= self._model_client,
                                      termination_condition = self.termination_condition,
                                      max_turns=self._node_param.manager.max_turns,
                                      selector_prompt=self._node_param.manager.selector_prompt,
    ...
    async def execute(self, context:Context) -> Response:
        response : Response = await Console(self.execute_stream(context), output_stats=True)
        return response
    ...
    async def execute_stream(self, context:Context) ->  AsyncGenerator[Union[BaseAgentEvent | BaseChatMessage | TaskResult | Response], None]:
        ...
        for i in range(max_iter):
            ...
            if self.use_check:
                check_result : CheckResult = await self.gen_check_result(msgs, cancellation_token=cancellation_token)
