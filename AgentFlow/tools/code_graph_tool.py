...
def construct_code_graph(
    ...
    include_dirs: Annotated[List[str], "The absolute include directories where the header files will be searched"] = [],
    source_dirs: Annotated[List[str], "The source files in these absolute directories will be parsed into Translation Units"] =[],
    ...
    if scopes:
        if isinstance(scopes, list) and len(scopes) > 0:
            if os.path.exists(scopes[0]):
                dirs = scopes
                scopes = {"dirs": dirs}
    else:
        scopes = {"dirs": [project_dir]}            
    ...
    try:
        ...
        return f"The project `{project_dir}` has been parsed successfully: number of nodes: {code_graph.graph.number_of_nodes()}, number of edges: {code_graph.graph.number_of_edges()}"                       
...
def get_translation_tasks_topologically(reverse: Annotated[bool, "A boolean parameter to control the topological sorting order"])->List[set]:
    ...
    """
    global code_graph
    sccs = code_graph.get_tasks_topologically(reverse)
    return sccs
